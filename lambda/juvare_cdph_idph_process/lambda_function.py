...
def csv_from_excel(filename, bucket_landing, job_id):
    ...
    try:
        ...
        if is_hash_found:
            ...
            dynamodb_helper.update_dynamodb_log(job_id, "EXCEPTION", "DUPLICATED CDPH IDPH")
            raise Exception("DUPLICATED: Juvare CDPH IDPH File was Already imported")
        ...
        for s in sheets:
            LOGGER.info(f"---- Juvare: Processing CDPH IDPH - {s} - {f_name_date} ----")
            # load the sheet name
            ...
            LOGGER.info(f"---- Juvare: CDPH IDPH SAVED in S3 - {s} - {f_name_date} ----")

        return True

    except Exception as err:
        LOGGER.error(f"## JUVARE CDPH IDPH PROCESSING EXCEPTION: {str(err)}")
        # send to SNS topic
        SNS_CLIENT.publish(TopicArn=SNS_TOPIC_ARN, Message=str(err), Subject="JUVARE CDPH IDPH PROCESSING EXCEPTION")
        raise err
...
def lambda_handler(event, context):
    LOGGER.info("---- JUVARE CDPH IDPH START PROCESSING ---")
    LOGGER.info(event)
    ...
    try:
        ...
        LOGGER.error(f"---- JUVARE CDPH IDPH EXCEPTION: {str(err)}")
        LOGGER.info(event)
        status = "FAILED"
