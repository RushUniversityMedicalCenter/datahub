"""
File: dynamodb_helper.py
Project: aws-rush-fhir
File Created: Thursday, 28th January 2021 8:56:23 am
Author: Canivel, Danilo (dccanive@amazon.com)
-----
Last Modified: Thursday, 28th January 2021 5:37:23 pm
...
DYNAMODB_HHS_HASH_TABLE_LOG = os.environ["DYNAMODB_HHS_HASH_TABLE_LOG"]
DYNAMODB_HHS_EXECUTION_LOG = os.environ["DYNAMODB_HHS_EXECUTION_LOG"]


def put_dynamodb_log(lambdaId, filename, status, creation_date, event, error_result):
    try:
        DYNAMODB_CLIENT.put_item(
            TableName=DYNAMODB_HHS_EXECUTION_LOG,
            Item={
                "lambdaId": {"S": lambdaId},
                "filename": {"S": filename},
...
def put_dynamodb_hash(lambdaId, creation_date, ccd_hash, filename):
    try:
        DYNAMODB_CLIENT.put_item(
            TableName=DYNAMODB_HHS_HASH_TABLE_LOG,
            Item={
...
def is_hash_existent(lambdaId, md5_digest, filename) -> bool:
    ...
    """
    try:
        response = DYNAMODB_CLIENT.get_item(TableName=DYNAMODB_HHS_HASH_TABLE_LOG, Key={"md5Digest": {"S": md5_digest}})
        ...
        if "Item" in response:
            ...
            message_log_response = DYNAMODB_CLIENT.get_item(
                TableName=DYNAMODB_HHS_EXECUTION_LOG, Key={"lambdaId": {"S": lambdaId_hash}}
            )
