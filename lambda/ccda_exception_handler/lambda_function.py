...
-----
Last Modified: Tuesday, 30th December 2020 2:37:35 pm
...
def save_dynamodb_log(aws_request_id, sqs_message_id, source_event, creation_date, error_type):
    try:
        DYNAMODB_CLIENT.put_item(TableName=CCDS_SFN_EXCEPTIONS_LOG,
                                 Item={'id': {'S': sqs_message_id},
...
def delete_hash_log(md5_digest):
    try:
        DYNAMODB_CLIENT.delete_item(TableName=CCDS_HASH_TABLE_LOG,
                                    Item={'ccd_hash': {'S': md5_digest}})
    except Exception as e:
        LOGGER.error(f'## DYNAMODB PUT MESSAGEID EXCEPTION: {str(e)}')
...
def lambda_handler(event, context):
    ...
    if error_type:
        ...
        if error_type == 'CCDADuplicatedError':
            # @TODO Send message to Dead-Letter with Status Duplicated
            save_dynamodb_log(aws_request_id, sqs_message_id,
                              source_event, creation_date, error_type)

        error_list_delete = ['ConverterError', 'FhirDatasetsGenerationError',
                             'HealthLakePostError', 'AWSKeyMissingError']
        if error_type in error_list_delete:
            # @TODO Send message to Dead-Letter with Error Status
            md5_digest = source_event['Object']['md5_digest']
            save_dynamodb_log(aws_request_id, sqs_message_id,
                              source_event, creation_date, error_type)
            delete_hash_log(md5_digest)
