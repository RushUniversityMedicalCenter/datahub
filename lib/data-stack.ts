...
export class dataStack extends Stack {
  constructor(app: App, id: string, props: dataStackProps) {
    ...
    const validateMapChain = sfn.Chain
      .start(new tasks.LambdaInvoke(this,'ValidateFile',{
        lambdaFunction: ccda_step2_validation.lambdaFunction,
      }).addCatch(exceptionHandler))
      .next(new tasks.LambdaInvoke(this,'Deduplication',{
        lambdaFunction: ccda_step3_deduplication.lambdaFunction
      }).addCatch(exceptionHandler))
      .next(new tasks.LambdaInvoke(this,'ConvertToFHIR',{
        lambdaFunction: ccda_step4_converter // this is direct lambda.Function not from createLambda function
      }).addCatch(exceptionHandler))
      .next(new tasks.LambdaInvoke(this, 'BuildFHIRDatasets',{
        lambdaFunction: ccda_step5_dataset_builder.lambdaFunction
      }).addCatch(exceptionHandler))
    ...
    const definition = sfn.Chain
      .start(new tasks.LambdaInvoke(this, 'ProcessSQSMessage', {
        lambdaFunction: ccda_step1_new_files.lambdaFunction,
        outputPath: '$.Payload',
      }))
      .next(validateMap)
