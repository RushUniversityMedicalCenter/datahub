 ...
 * -----
 * Last Modified: Friday, 29th January 2021 9:03:52 am
...
export class fhirStack extends Stack {
  constructor(app: App, id: string, props: fhirStackProps) {
    ...
    roleLambdaProcessCCD.addManagedPolicy(
    ...
    healthLakePolicy.addStatements(
    ...
    s3LandingDirect.bucket.addEventNotification(
      s3.EventType.OBJECT_CREATED,
      new s3n.SqsDestination(ccdQueue)
    );
    s3LandingApi.bucket.addEventNotification(
      s3.EventType.OBJECT_CREATED,
      new s3n.SqsDestination(ccdQueue)
    );
    s3LandingSftp.bucket.addEventNotification(
      s3.EventType.OBJECT_CREATED,
      new s3n.SqsDestination(ccdQueue)
    );
    ...
    const ccda_step1_new_files = new createLambda(
      this,
      envName,
      roleLambdaProcessCCD,
      "ccda_step1_new_files",
      {
    ...
    const ccda_step2_validation = new createLambda(
      this,
      envName,
      roleLambdaProcessCCD,
      "ccda_step2_validation",
      {
        CCDS_SQSMESSAGE_TABLE_LOG: ccds_sqs_messages_log.tableName,
      }
    );

    const ccda_step3_deduplication = new createLambda(
      this,
      envName,
      roleLambdaProcessCCD,
      "ccda_step3_deduplication",
      {
    ...
    const ccda_step5_dataset_builder = new createLambdaWithLayer(
      this,
      envName,
      roleLambdaProcessCCD,
      "ccda_step5_dataset_builder",
      layerWrangler,
      {
    ...
    const ccda_step6_fhir_resource_split = new createLambdaWithLayer(
      this,
      envName,
      roleLambdaProcessCCD,
      "ccda_step6_fhir_resource_split",
      layerWrangler,
      {
    ...
    const ccda_exception_handler = new createLambda(
      this,
      envName,
      roleLambdaProcessCCD,
      "ccda_exception_handler",
      {
        CCDS_HASH_TABLE_LOG: ccds_hash_table_log.tableName,
    ...
    const ccda_finish_stepfunction = new createLambda(
      this,
      envName,
      roleLambdaProcessCCD,
      "ccda_finish_stepfunction",
      {
    ...
    const exceptionHandler = sfn.Chain.start(ExceptionHandler).next(NotifyFailure).next(Fail);
    ...
    const validateMapChain = sfn.Chain.start(ValidateFile)
      .next(Deduplication)
      .next(ConvertToFHIR)
      ...
      .next(WaitTryAgain)
      .next(
        new sfn.Choice(this, "FHIRComplete?")
          .when(sfn.Condition.stringEquals("$.Status", "COMPLETED"), Complete)
          .when(
            sfn.Condition.stringEquals("$.Error", "HealthLakePostTooManyRequestsError"),
            SaveFHIRResources
          )
      );
    ...
    const definition = sfn.Chain.start(ProcessSQSMessage).next(validateMap).next(FinalizeProcess);
    ...
    const ccda_step0_start_state_machine = new createLambda(
      this,
      envName,
      roleLambdaProcessCCD,
      "ccda_step0_start_state_machine",
      {
        CCDS_SQSMESSAGE_TABLE_LOG: ccds_sqs_messages_log.tableName,
    ...
    // Lambda Triggers
    ccda_step0_start_state_machine.lambdaFunction.addEventSource(
      new SqsEventSource(ccdQueue, {
        batchSize: 1,
        enabled: true,
      })
    );
    ...
    const lambdaUploadCCDApi = new createLambda(
      this,
      envName,
      roleLambdaProcessCCD,
      "uploadCCDApi",
